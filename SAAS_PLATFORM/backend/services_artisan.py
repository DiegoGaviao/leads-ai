...
def _fallback_dalle_prompt(post: dict, brand_tone: str, audience_dna: str) -> str:
    ...
    parts = [
        "Premium editorial photograph for Instagram vertical content, cinematic lighting, shallow depth of field.",
    ...
    if body and not visual:
        parts.append(f"Narrative mood (no readable text in image): {body[:400]}")
    parts.append("Photorealistic, tasteful, no text overlays, no logos, no watermarks.")
    ...
    def enrich_report_with_images(
        ...
        *,
        ...
        for i, post in enumerate(strategy_json["roteiros"]):
            ...
            try:
                prompts = artisan.generate_visual_prompts(script_text, brand_tone, audience_dna)
            ...
            if not primary:
                primary = _fallback_dalle_prompt(post, brand_tone, audience_dna)
